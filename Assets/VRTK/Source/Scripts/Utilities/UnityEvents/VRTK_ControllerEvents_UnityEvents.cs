namespace VRTK.UnityEventHelper
{
    ...
    {     
        ...
        public class Entry
        {
            public ControllerInteractionType eventID = ControllerInteractionType.TriggerPressed;
            public ControllerInteractionEvent callback = new ControllerInteractionEvent();
        }
...
#region Events
        ...
        // TODO: eventArg hierachy?
        private void Execute(ControllerInteractionType id, object o, ControllerInteractionEventArgs e)
        {
            if (delegates != null)
            {
                foreach (Entry entry in delegates)
                {
                    if (entry.eventID == id && entry.callback != null)
                    {
                        entry.callback.Invoke(o, e);
                    }
                }                
            }
        }
